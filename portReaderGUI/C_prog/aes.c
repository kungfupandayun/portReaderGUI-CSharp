/******************************************
** Advanced Encryption Standard
** Author: B-Con (b-con@b-con.us)
** Copyright/Restrictions: GNU GPL
** Disclaimer: This code is presented "as is" without any garuentees; said author holds
               liability for no problems rendered by the use of this code.
** Details: This code is the implementation of the AES algorithm, as specified by the
            NIST in in publication FIPS PUB 197, availible on the NIST website at
            http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf .
******************************************/

#include <stdio.h>
#include <string.h>
#include "aes.h"



// Alternative solution for S-Box
extern unsigned int Alt_IsSet;
extern unsigned char Alt_Faults_Value[17]; // Bit-flip value
extern unsigned int Alt_Fault_Position;
extern unsigned char Alt_Fault_Value;
extern unsigned int Alt_Num_of_BitFlips;



// #define uchar unsigned char // 8-bit byte
// #define uint unsigned long // 32-bit word

// This is the specified AES SBox. To look up a substitution value, put the first
// nibble in the first index (row) and the second nibble in the second index (column).
const uchar aes_sbox[16][16] = {
   
};

const uchar aes_invsbox[16][16] = {
   
};

// - This table stores pre-calculated values for all possible GF(2^8) calculations.This
// table is only used by the (Inv)MixColumns steps.
// USAGE: The second index (column) is the coefficient of multiplication. Only 7 different
// coefficients are used: 0x01, 0x02, 0x03, 0x09, 0x0b, 0x0d, 0x0e, but multiplication by
// 1 is negligible leaving only 6 coefficients. Each column of the table is devoted to one
// of these coefficients, in the ascending order of value, from values 0x00 to 0xFF.
// (Columns are listed double-wide to conserve vertical space.)
uchar gf_mul[256][6] = {
   {0x00,0x00,0x00,0x00,0x00,0x00},{0x02,0x03,0x09,0x0b,0x0d,0x0e},
   {0x04,0x06,0x12,0x16,0x1a,0x1c},{0x06,0x05,0x1b,0x1d,0x17,0x12},
   {0x08,0x0c,0x24,0x2c,0x34,0x38},{0x0a,0x0f,0x2d,0x27,0x39,0x36},
   {0x0c,0x0a,0x36,0x3a,0x2e,0x24},{0x0e,0x09,0x3f,0x31,0x23,0x2a},
   {0x10,0x18,0x48,0x58,0x68,0x70},{0x12,0x1b,0x41,0x53,0x65,0x7e},
   {0x14,0x1e,0x5a,0x4e,0x72,0x6c},{0x16,0x1d,0x53,0x45,0x7f,0x62},
   {0x18,0x14,0x6c,0x74,0x5c,0x48},{0x1a,0x17,0x65,0x7f,0x51,0x46},
   {0x1c,0x12,0x7e,0x62,0x46,0x54},{0x1e,0x11,0x77,0x69,0x4b,0x5a},
   {0x20,0x30,0x90,0xb0,0xd0,0xe0},{0x22,0x33,0x99,0xbb,0xdd,0xee},
   {0x24,0x36,0x82,0xa6,0xca,0xfc},{0x26,0x35,0x8b,0xad,0xc7,0xf2},
   {0x28,0x3c,0xb4,0x9c,0xe4,0xd8},{0x2a,0x3f,0xbd,0x97,0xe9,0xd6},
   {0x2c,0x3a,0xa6,0x8a,0xfe,0xc4},{0x2e,0x39,0xaf,0x81,0xf3,0xca},
   {0x30,0x28,0xd8,0xe8,0xb8,0x90},{0x32,0x2b,0xd1,0xe3,0xb5,0x9e},
   {0x34,0x2e,0xca,0xfe,0xa2,0x8c},{0x36,0x2d,0xc3,0xf5,0xaf,0x82},
   {0x38,0x24,0xfc,0xc4,0x8c,0xa8},{0x3a,0x27,0xf5,0xcf,0x81,0xa6},
   {0x3c,0x22,0xee,0xd2,0x96,0xb4},{0x3e,0x21,0xe7,0xd9,0x9b,0xba},
   {0x40,0x60,0x3b,0x7b,0xbb,0xdb},{0x42,0x63,0x32,0x70,0xb6,0xd5},
   {0x44,0x66,0x29,0x6d,0xa1,0xc7},{0x46,0x65,0x20,0x66,0xac,0xc9},
   {0x48,0x6c,0x1f,0x57,0x8f,0xe3},{0x4a,0x6f,0x16,0x5c,0x82,0xed},
   {0x4c,0x6a,0x0d,0x41,0x95,0xff},{0x4e,0x69,0x04,0x4a,0x98,0xf1},
   {0x50,0x78,0x73,0x23,0xd3,0xab},{0x52,0x7b,0x7a,0x28,0xde,0xa5},
   {0x54,0x7e,0x61,0x35,0xc9,0xb7},{0x56,0x7d,0x68,0x3e,0xc4,0xb9},
   {0x58,0x74,0x57,0x0f,0xe7,0x93},{0x5a,0x77,0x5e,0x04,0xea,0x9d},
   {0x5c,0x72,0x45,0x19,0xfd,0x8f},{0x5e,0x71,0x4c,0x12,0xf0,0x81},
   {0x60,0x50,0xab,0xcb,0x6b,0x3b},{0x62,0x53,0xa2,0xc0,0x66,0x35},
   {0x64,0x56,0xb9,0xdd,0x71,0x27},{0x66,0x55,0xb0,0xd6,0x7c,0x29},
   {0x68,0x5c,0x8f,0xe7,0x5f,0x03},{0x6a,0x5f,0x86,0xec,0x52,0x0d},
   {0x6c,0x5a,0x9d,0xf1,0x45,0x1f},{0x6e,0x59,0x94,0xfa,0x48,0x11},
   {0x70,0x48,0xe3,0x93,0x03,0x4b},{0x72,0x4b,0xea,0x98,0x0e,0x45},
   {0x74,0x4e,0xf1,0x85,0x19,0x57},{0x76,0x4d,0xf8,0x8e,0x14,0x59},
   {0x78,0x44,0xc7,0xbf,0x37,0x73},{0x7a,0x47,0xce,0xb4,0x3a,0x7d},
   {0x7c,0x42,0xd5,0xa9,0x2d,0x6f},{0x7e,0x41,0xdc,0xa2,0x20,0x61},
   {0x80,0xc0,0x76,0xf6,0x6d,0xad},{0x82,0xc3,0x7f,0xfd,0x60,0xa3},
   {0x84,0xc6,0x64,0xe0,0x77,0xb1},{0x86,0xc5,0x6d,0xeb,0x7a,0xbf},
   {0x88,0xcc,0x52,0xda,0x59,0x95},{0x8a,0xcf,0x5b,0xd1,0x54,0x9b},
   {0x8c,0xca,0x40,0xcc,0x43,0x89},{0x8e,0xc9,0x49,0xc7,0x4e,0x87},
   {0x90,0xd8,0x3e,0xae,0x05,0xdd},{0x92,0xdb,0x37,0xa5,0x08,0xd3},
   {0x94,0xde,0x2c,0xb8,0x1f,0xc1},{0x96,0xdd,0x25,0xb3,0x12,0xcf},
   {0x98,0xd4,0x1a,0x82,0x31,0xe5},{0x9a,0xd7,0x13,0x89,0x3c,0xeb},
   {0x9c,0xd2,0x08,0x94,0x2b,0xf9},{0x9e,0xd1,0x01,0x9f,0x26,0xf7},
   {0xa0,0xf0,0xe6,0x46,0xbd,0x4d},{0xa2,0xf3,0xef,0x4d,0xb0,0x43},
   {0xa4,0xf6,0xf4,0x50,0xa7,0x51},{0xa6,0xf5,0xfd,0x5b,0xaa,0x5f},
   {0xa8,0xfc,0xc2,0x6a,0x89,0x75},{0xaa,0xff,0xcb,0x61,0x84,0x7b},
   {0xac,0xfa,0xd0,0x7c,0x93,0x69},{0xae,0xf9,0xd9,0x77,0x9e,0x67},
   {0xb0,0xe8,0xae,0x1e,0xd5,0x3d},{0xb2,0xeb,0xa7,0x15,0xd8,0x33},
   {0xb4,0xee,0xbc,0x08,0xcf,0x21},{0xb6,0xed,0xb5,0x03,0xc2,0x2f},
   {0xb8,0xe4,0x8a,0x32,0xe1,0x05},{0xba,0xe7,0x83,0x39,0xec,0x0b},
   {0xbc,0xe2,0x98,0x24,0xfb,0x19},{0xbe,0xe1,0x91,0x2f,0xf6,0x17},
   {0xc0,0xa0,0x4d,0x8d,0xd6,0x76},{0xc2,0xa3,0x44,0x86,0xdb,0x78},
   {0xc4,0xa6,0x5f,0x9b,0xcc,0x6a},{0xc6,0xa5,0x56,0x90,0xc1,0x64},
   {0xc8,0xac,0x69,0xa1,0xe2,0x4e},{0xca,0xaf,0x60,0xaa,0xef,0x40},
   {0xcc,0xaa,0x7b,0xb7,0xf8,0x52},{0xce,0xa9,0x72,0xbc,0xf5,0x5c},
   {0xd0,0xb8,0x05,0xd5,0xbe,0x06},{0xd2,0xbb,0x0c,0xde,0xb3,0x08},
   {0xd4,0xbe,0x17,0xc3,0xa4,0x1a},{0xd6,0xbd,0x1e,0xc8,0xa9,0x14},
   {0xd8,0xb4,0x21,0xf9,0x8a,0x3e},{0xda,0xb7,0x28,0xf2,0x87,0x30},
   {0xdc,0xb2,0x33,0xef,0x90,0x22},{0xde,0xb1,0x3a,0xe4,0x9d,0x2c},
   {0xe0,0x90,0xdd,0x3d,0x06,0x96},{0xe2,0x93,0xd4,0x36,0x0b,0x98},
   {0xe4,0x96,0xcf,0x2b,0x1c,0x8a},{0xe6,0x95,0xc6,0x20,0x11,0x84},
   {0xe8,0x9c,0xf9,0x11,0x32,0xae},{0xea,0x9f,0xf0,0x1a,0x3f,0xa0},
   {0xec,0x9a,0xeb,0x07,0x28,0xb2},{0xee,0x99,0xe2,0x0c,0x25,0xbc},
   {0xf0,0x88,0x95,0x65,0x6e,0xe6},{0xf2,0x8b,0x9c,0x6e,0x63,0xe8},
   {0xf4,0x8e,0x87,0x73,0x74,0xfa},{0xf6,0x8d,0x8e,0x78,0x79,0xf4},
   {0xf8,0x84,0xb1,0x49,0x5a,0xde},{0xfa,0x87,0xb8,0x42,0x57,0xd0},
   {0xfc,0x82,0xa3,0x5f,0x40,0xc2},{0xfe,0x81,0xaa,0x54,0x4d,0xcc},
   {0x1b,0x9b,0xec,0xf7,0xda,0x41},{0x19,0x98,0xe5,0xfc,0xd7,0x4f},
   {0x1f,0x9d,0xfe,0xe1,0xc0,0x5d},{0x1d,0x9e,0xf7,0xea,0xcd,0x53},
   {0x13,0x97,0xc8,0xdb,0xee,0x79},{0x11,0x94,0xc1,0xd0,0xe3,0x77},
   {0x17,0x91,0xda,0xcd,0xf4,0x65},{0x15,0x92,0xd3,0xc6,0xf9,0x6b},
   {0x0b,0x83,0xa4,0xaf,0xb2,0x31},{0x09,0x80,0xad,0xa4,0xbf,0x3f},
   {0x0f,0x85,0xb6,0xb9,0xa8,0x2d},{0x0d,0x86,0xbf,0xb2,0xa5,0x23},
   {0x03,0x8f,0x80,0x83,0x86,0x09},{0x01,0x8c,0x89,0x88,0x8b,0x07},
   {0x07,0x89,0x92,0x95,0x9c,0x15},{0x05,0x8a,0x9b,0x9e,0x91,0x1b},
   {0x3b,0xab,0x7c,0x47,0x0a,0xa1},{0x39,0xa8,0x75,0x4c,0x07,0xaf},
   {0x3f,0xad,0x6e,0x51,0x10,0xbd},{0x3d,0xae,0x67,0x5a,0x1d,0xb3},
   {0x33,0xa7,0x58,0x6b,0x3e,0x99},{0x31,0xa4,0x51,0x60,0x33,0x97},
   {0x37,0xa1,0x4a,0x7d,0x24,0x85},{0x35,0xa2,0x43,0x76,0x29,0x8b},
   {0x2b,0xb3,0x34,0x1f,0x62,0xd1},{0x29,0xb0,0x3d,0x14,0x6f,0xdf},
   {0x2f,0xb5,0x26,0x09,0x78,0xcd},{0x2d,0xb6,0x2f,0x02,0x75,0xc3},
   {0x23,0xbf,0x10,0x33,0x56,0xe9},{0x21,0xbc,0x19,0x38,0x5b,0xe7},
   {0x27,0xb9,0x02,0x25,0x4c,0xf5},{0x25,0xba,0x0b,0x2e,0x41,0xfb},
   {0x5b,0xfb,0xd7,0x8c,0x61,0x9a},{0x59,0xf8,0xde,0x87,0x6c,0x94},
   {0x5f,0xfd,0xc5,0x9a,0x7b,0x86},{0x5d,0xfe,0xcc,0x91,0x76,0x88},
   {0x53,0xf7,0xf3,0xa0,0x55,0xa2},{0x51,0xf4,0xfa,0xab,0x58,0xac},
   {0x57,0xf1,0xe1,0xb6,0x4f,0xbe},{0x55,0xf2,0xe8,0xbd,0x42,0xb0},
   {0x4b,0xe3,0x9f,0xd4,0x09,0xea},{0x49,0xe0,0x96,0xdf,0x04,0xe4},
   {0x4f,0xe5,0x8d,0xc2,0x13,0xf6},{0x4d,0xe6,0x84,0xc9,0x1e,0xf8},
   {0x43,0xef,0xbb,0xf8,0x3d,0xd2},{0x41,0xec,0xb2,0xf3,0x30,0xdc},
   {0x47,0xe9,0xa9,0xee,0x27,0xce},{0x45,0xea,0xa0,0xe5,0x2a,0xc0},
   {0x7b,0xcb,0x47,0x3c,0xb1,0x7a},{0x79,0xc8,0x4e,0x37,0xbc,0x74},
   {0x7f,0xcd,0x55,0x2a,0xab,0x66},{0x7d,0xce,0x5c,0x21,0xa6,0x68},
   {0x73,0xc7,0x63,0x10,0x85,0x42},{0x71,0xc4,0x6a,0x1b,0x88,0x4c},
   {0x77,0xc1,0x71,0x06,0x9f,0x5e},{0x75,0xc2,0x78,0x0d,0x92,0x50},
   {0x6b,0xd3,0x0f,0x64,0xd9,0x0a},{0x69,0xd0,0x06,0x6f,0xd4,0x04},
   {0x6f,0xd5,0x1d,0x72,0xc3,0x16},{0x6d,0xd6,0x14,0x79,0xce,0x18},
   {0x63,0xdf,0x2b,0x48,0xed,0x32},{0x61,0xdc,0x22,0x43,0xe0,0x3c},
   {0x67,0xd9,0x39,0x5e,0xf7,0x2e},{0x65,0xda,0x30,0x55,0xfa,0x20},
   {0x9b,0x5b,0x9a,0x01,0xb7,0xec},{0x99,0x58,0x93,0x0a,0xba,0xe2},
   {0x9f,0x5d,0x88,0x17,0xad,0xf0},{0x9d,0x5e,0x81,0x1c,0xa0,0xfe},
   {0x93,0x57,0xbe,0x2d,0x83,0xd4},{0x91,0x54,0xb7,0x26,0x8e,0xda},
   {0x97,0x51,0xac,0x3b,0x99,0xc8},{0x95,0x52,0xa5,0x30,0x94,0xc6},
   {0x8b,0x43,0xd2,0x59,0xdf,0x9c},{0x89,0x40,0xdb,0x52,0xd2,0x92},
   {0x8f,0x45,0xc0,0x4f,0xc5,0x80},{0x8d,0x46,0xc9,0x44,0xc8,0x8e},
   {0x83,0x4f,0xf6,0x75,0xeb,0xa4},{0x81,0x4c,0xff,0x7e,0xe6,0xaa},
   {0x87,0x49,0xe4,0x63,0xf1,0xb8},{0x85,0x4a,0xed,0x68,0xfc,0xb6},
   {0xbb,0x6b,0x0a,0xb1,0x67,0x0c},{0xb9,0x68,0x03,0xba,0x6a,0x02},
   {0xbf,0x6d,0x18,0xa7,0x7d,0x10},{0xbd,0x6e,0x11,0xac,0x70,0x1e},
   {0xb3,0x67,0x2e,0x9d,0x53,0x34},{0xb1,0x64,0x27,0x96,0x5e,0x3a},
   {0xb7,0x61,0x3c,0x8b,0x49,0x28},{0xb5,0x62,0x35,0x80,0x44,0x26},
   {0xab,0x73,0x42,0xe9,0x0f,0x7c},{0xa9,0x70,0x4b,0xe2,0x02,0x72},
   {0xaf,0x75,0x50,0xff,0x15,0x60},{0xad,0x76,0x59,0xf4,0x18,0x6e},
   {0xa3,0x7f,0x66,0xc5,0x3b,0x44},{0xa1,0x7c,0x6f,0xce,0x36,0x4a},
   {0xa7,0x79,0x74,0xd3,0x21,0x58},{0xa5,0x7a,0x7d,0xd8,0x2c,0x56},
   {0xdb,0x3b,0xa1,0x7a,0x0c,0x37},{0xd9,0x38,0xa8,0x71,0x01,0x39},
   {0xdf,0x3d,0xb3,0x6c,0x16,0x2b},{0xdd,0x3e,0xba,0x67,0x1b,0x25},
   {0xd3,0x37,0x85,0x56,0x38,0x0f},{0xd1,0x34,0x8c,0x5d,0x35,0x01},
   {0xd7,0x31,0x97,0x40,0x22,0x13},{0xd5,0x32,0x9e,0x4b,0x2f,0x1d},
   {0xcb,0x23,0xe9,0x22,0x64,0x47},{0xc9,0x20,0xe0,0x29,0x69,0x49},
   {0xcf,0x25,0xfb,0x34,0x7e,0x5b},{0xcd,0x26,0xf2,0x3f,0x73,0x55},
   {0xc3,0x2f,0xcd,0x0e,0x50,0x7f},{0xc1,0x2c,0xc4,0x05,0x5d,0x71},
   {0xc7,0x29,0xdf,0x18,0x4a,0x63},{0xc5,0x2a,0xd6,0x13,0x47,0x6d},
   {0xfb,0x0b,0x31,0xca,0xdc,0xd7},{0xf9,0x08,0x38,0xc1,0xd1,0xd9},
   {0xff,0x0d,0x23,0xdc,0xc6,0xcb},{0xfd,0x0e,0x2a,0xd7,0xcb,0xc5},
   {0xf3,0x07,0x15,0xe6,0xe8,0xef},{0xf1,0x04,0x1c,0xed,0xe5,0xe1},
   {0xf7,0x01,0x07,0xf0,0xf2,0xf3},{0xf5,0x02,0x0e,0xfb,0xff,0xfd},
   {0xeb,0x13,0x79,0x92,0xb4,0xa7},{0xe9,0x10,0x70,0x99,0xb9,0xa9},
   {0xef,0x15,0x6b,0x84,0xae,0xbb},{0xed,0x16,0x62,0x8f,0xa3,0xb5},
   {0xe3,0x1f,0x5d,0xbe,0x80,0x9f},{0xe1,0x1c,0x54,0xb5,0x8d,0x91},
   {0xe7,0x19,0x4f,0xa8,0x9a,0x83},{0xe5,0x1a,0x46,0xa3,0x97,0x8d}
};


char* trimspaces( char* str_out, const char* str_in )
{
    int i;
    int l=strlen( str_in );
    strcpy( str_out, str_in );
    for ( i=0; i<l; i++ )
        if ( str_out[i]==' ' )
            strncpy( str_out+i, str_out+i+1, l-i-1 );
    return str_out;
}


void Input2State( uchar dest[][4], uchar src[] )
// Copy and rearrange one string of 16 unsigned char (bytes) to the square state matrix
{
   dest[0][0] = src[0];  dest[0][1] = src[4];   dest[0][2] = src[8];   dest[0][3] = src[12];
   dest[1][0] = src[1];  dest[1][1] = src[5];   dest[1][2] = src[9];   dest[1][3] = src[13];
   dest[2][0] = src[2];  dest[2][1] = src[6];   dest[2][2] = src[10];  dest[2][3] = src[14];
   dest[3][0] = src[3];  dest[3][1] = src[7];   dest[3][2] = src[11];  dest[3][3] = src[15];
}

void next_key( uchar rk[][4], uchar *rc )
{
   // uchar nk[4][4];
   uchar swi[4], swo[4], krco[4];
	int i, row, col;

	swi[0] = rk[1][3];
	swi[1] = rk[2][3];
	swi[2] = rk[3][3];
	swi[3] = rk[0][3];

	for ( i=0; i<4; i++ )
		swo[i] = aes_sbox[(swi[i] >> 4) & 0x0000000F][swi[i] & 0x0000000F];

	krco[0] = swo[0] ^ (*rc);
	*rc = ( (*rc) & 0x00000080 ) ? (( (*rc) << 1 )^0x1b) : ( (*rc) << 1 );
	for ( i=1; i<4; i++ ) krco[i] = swo[i] ;

	for ( row=0; row<4; row++ )
      // nk[row][0] = rk[row][0] ^ krco[row];
      rk[row][0] = rk[row][0] ^ krco[row];
	for ( col=1; col<4; col++ )
		for ( row=0; row<4; row++ )
         // nk[row][col] = rk[row][col] ^ nk[row][col-1];
         rk[row][col] = rk[row][col] ^ rk[row][col-1];
}

void next_key_F( uchar rk[][4], uchar *rc, unsigned int sbox_in_flt )
{
   // uchar nk[4][4];
   uchar swi[4], swo[4], krco[4];
	int i, row, col;

	swi[0] = rk[1][3] ^ ( (sbox_in_flt      ) & 0xFF );
	swi[1] = rk[2][3] ^ ( (sbox_in_flt >>  8) & 0xFF );
	swi[2] = rk[3][3] ^ ( (sbox_in_flt >> 16) & 0xFF );
	swi[3] = rk[0][3] ^ ( (sbox_in_flt >> 24) & 0xFF );

	for ( i=0; i<4; i++ )
		swo[i] = aes_sbox[(swi[i] >> 4) & 0x0000000F][swi[i] & 0x0000000F];

	krco[0] = swo[0] ^ (*rc);
	*rc = ( (*rc) & 0x00000080 ) ? (( (*rc) << 1 )^0x1b) : ( (*rc) << 1 );
	for ( i=1; i<4; i++ ) krco[i] = swo[i] ;

	for ( row=0; row<4; row++ )
      // nk[row][0] = rk[row][0] ^ krco[row];
      rk[row][0] = rk[row][0] ^ krco[row];
	for ( col=1; col<4; col++ )
		for ( row=0; row<4; row++ )
         // nk[row][col] = rk[row][col] ^ nk[row][col-1];
         rk[row][col] = rk[row][col] ^ rk[row][col-1];
}


void prev_key( uchar nk[][4], uchar *rc )
{
   uchar rk[4][4];
   uchar swi[4], swo[4], krco[4];
	int i, row, col;

	// Rebuild last three columns of previous round key:
	for ( col=3; col>0; col-- )
		for ( row=0; row<4; row++ )
			rk[row][col] = nk[row][col] ^ nk[row][col-1];

	// rotword
	swi[0] = rk[1][3];
	swi[1] = rk[2][3];
	swi[2] = rk[3][3];
	swi[3] = rk[0][3];

	// subword
	for ( i=0; i<4; i++ )
		swo[i] = aes_sbox[(swi[i] >> 4) & 0x0000000F][swi[i] & 0x0000000F];

	(*rc) = ( (*rc) & 0x00000001 ) ? (( (*rc)>>1 )^0x8d) : ( (*rc) >> 1 );
	krco[0] = swo[0] ^ (*rc);
	for ( i=1; i<4; i++ ) krco[i] = swo[i] ;

	for ( row=0; row<4; row++ )
      rk[row][0] = nk[row][0] ^ krco[row];
	for ( col=0; col<4; col++ )
		for ( row=0; row<4; row++ )
         nk[row][col] = rk[row][col];
}


void AddKey( uchar t[][4], uchar k[][4] )
{
   int r, c;
   for ( r=0; r<4; r++ )
      for ( c=0; c<4; c++ )
         t[r][c] = t[r][c] ^ k[r][c];
}



/********************
** ADD ROUND KEY
********************/

// Performs the AddRoundKey step. Each round has its own pre-generated 16-byte key in the
// form of 4 integers (the "w" array). Each integer is XOR'd by one column of the state.
// Also performs the job of InvAddRoundKey(); since the function is a simple XOR process,
// it is its own inverse.
void AddRoundKey(uchar state[][4], uint w[])
{
   uchar subkey[4];
   // memcpy(subkey,&w[idx],4); // Not accurate for big endian machines
   // Subkey 1
   subkey[0] = w[0] >> 24;
   subkey[1] = w[0] >> 16;
   subkey[2] = w[0] >> 8;
   subkey[3] = w[0];
   state[0][0] ^= subkey[0];
   state[1][0] ^= subkey[1];
   state[2][0] ^= subkey[2];
   state[3][0] ^= subkey[3];
   // Subkey 2
   subkey[0] = w[1] >> 24;
   subkey[1] = w[1] >> 16;
   subkey[2] = w[1] >> 8;
   subkey[3] = w[1];
   state[0][1] ^= subkey[0];
   state[1][1] ^= subkey[1];
   state[2][1] ^= subkey[2];
   state[3][1] ^= subkey[3];
   // Subkey 3
   subkey[0] = w[2] >> 24;
   subkey[1] = w[2] >> 16;
   subkey[2] = w[2] >> 8;
   subkey[3] = w[2];
   state[0][2] ^= subkey[0];
   state[1][2] ^= subkey[1];
   state[2][2] ^= subkey[2];
   state[3][2] ^= subkey[3];
   // Subkey 4
   subkey[0] = w[3] >> 24;
   subkey[1] = w[3] >> 16;
   subkey[2] = w[3] >> 8;
   subkey[3] = w[3];
   state[0][3] ^= subkey[0];
   state[1][3] ^= subkey[1];
   state[2][3] ^= subkey[2];
   state[3][3] ^= subkey[3];
}

/********************
** (Inv)SubBytes
********************/

// Performs the SubBytes step. All bytes in the state are substituted with a
// pre-calculated value from a lookup table.
void SubBytes(uchar state[][4])
{
   state[0][0] = aes_sbox[state[0][0] >> 4][state[0][0] & 0x0F];
   state[0][1] = aes_sbox[state[0][1] >> 4][state[0][1] & 0x0F];
   state[0][2] = aes_sbox[state[0][2] >> 4][state[0][2] & 0x0F];
   state[0][3] = aes_sbox[state[0][3] >> 4][state[0][3] & 0x0F];
   state[1][0] = aes_sbox[state[1][0] >> 4][state[1][0] & 0x0F];
   state[1][1] = aes_sbox[state[1][1] >> 4][state[1][1] & 0x0F];
   state[1][2] = aes_sbox[state[1][2] >> 4][state[1][2] & 0x0F];
   state[1][3] = aes_sbox[state[1][3] >> 4][state[1][3] & 0x0F];
   state[2][0] = aes_sbox[state[2][0] >> 4][state[2][0] & 0x0F];
   state[2][1] = aes_sbox[state[2][1] >> 4][state[2][1] & 0x0F];
   state[2][2] = aes_sbox[state[2][2] >> 4][state[2][2] & 0x0F];
   state[2][3] = aes_sbox[state[2][3] >> 4][state[2][3] & 0x0F];
   state[3][0] = aes_sbox[state[3][0] >> 4][state[3][0] & 0x0F];
   state[3][1] = aes_sbox[state[3][1] >> 4][state[3][1] & 0x0F];
   state[3][2] = aes_sbox[state[3][2] >> 4][state[3][2] & 0x0F];
   state[3][3] = aes_sbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

void InvSubBytes(uchar state[][4])
{
   state[0][0] = aes_invsbox[state[0][0] >> 4][state[0][0] & 0x0F];
   state[0][1] = aes_invsbox[state[0][1] >> 4][state[0][1] & 0x0F];
   state[0][2] = aes_invsbox[state[0][2] >> 4][state[0][2] & 0x0F];
   state[0][3] = aes_invsbox[state[0][3] >> 4][state[0][3] & 0x0F];
   state[1][0] = aes_invsbox[state[1][0] >> 4][state[1][0] & 0x0F];
   state[1][1] = aes_invsbox[state[1][1] >> 4][state[1][1] & 0x0F];
   state[1][2] = aes_invsbox[state[1][2] >> 4][state[1][2] & 0x0F];
   state[1][3] = aes_invsbox[state[1][3] >> 4][state[1][3] & 0x0F];
   state[2][0] = aes_invsbox[state[2][0] >> 4][state[2][0] & 0x0F];
   state[2][1] = aes_invsbox[state[2][1] >> 4][state[2][1] & 0x0F];
   state[2][2] = aes_invsbox[state[2][2] >> 4][state[2][2] & 0x0F];
   state[2][3] = aes_invsbox[state[2][3] >> 4][state[2][3] & 0x0F];
   state[3][0] = aes_invsbox[state[3][0] >> 4][state[3][0] & 0x0F];
   state[3][1] = aes_invsbox[state[3][1] >> 4][state[3][1] & 0x0F];
   state[3][2] = aes_invsbox[state[3][2] >> 4][state[3][2] & 0x0F];
   state[3][3] = aes_invsbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

/********************
** (Inv)ShiftRows
********************/

// Performs the ShiftRows step. All rows are shifted cylindrically to the left.
void ShiftRows(uchar state[][4])
{
   int t;
   // Shift left by 1
   t = state[1][0];
   state[1][0] = state[1][1];
   state[1][1] = state[1][2];
   state[1][2] = state[1][3];
   state[1][3] = t;
   // Shift left by 2
   t = state[2][0];
   state[2][0] = state[2][2];
   state[2][2] = t;
   t = state[2][1];
   state[2][1] = state[2][3];
   state[2][3] = t;
   // Shift left by 3
   t = state[3][0];
   state[3][0] = state[3][3];
   state[3][3] = state[3][2];
   state[3][2] = state[3][1];
   state[3][1] = t;
}

// All rows are shifted cylindrically to the right.
void InvShiftRows(uchar state[][4])
{
   int t;
   // Shift right by 1
   t = state[1][3];
   state[1][3] = state[1][2];
   state[1][2] = state[1][1];
   state[1][1] = state[1][0];
   state[1][0] = t;
   // Shift right by 2
   t = state[2][3];
   state[2][3] = state[2][1];
   state[2][1] = t;
   t = state[2][2];
   state[2][2] = state[2][0];
   state[2][0] = t;
   // Shift right by 3
   t = state[3][3];
   state[3][3] = state[3][0];
   state[3][0] = state[3][1];
   state[3][1] = state[3][2];
   state[3][2] = t;
}

/********************
** (Inv)MixColumns
********************/

// Performs the MixColums step. The state is multiplied by itself using matrix
// multiplication in a Galios Field 2^8. All multiplication is pre-computed in a table.
// Addition is equivilent to XOR. (Must always make a copy of the column as the original
// values will be destoyed.)
void MixColumns(uchar state[][4])
{
   uchar col[4];
   // Column 1
   col[0] = state[0][0];
   col[1] = state[1][0];
   col[2] = state[2][0];
   col[3] = state[3][0];
   state[0][0] = gf_mul[col[0]][0];
   state[0][0] ^= gf_mul[col[1]][1];
   state[0][0] ^= col[2];
   state[0][0] ^= col[3];
   state[1][0] = col[0];
   state[1][0] ^= gf_mul[col[1]][0];
   state[1][0] ^= gf_mul[col[2]][1];
   state[1][0] ^= col[3];
   state[2][0] = col[0];
   state[2][0] ^= col[1];
   state[2][0] ^= gf_mul[col[2]][0];
   state[2][0] ^= gf_mul[col[3]][1];
   state[3][0] = gf_mul[col[0]][1];
   state[3][0] ^= col[1];
   state[3][0] ^= col[2];
   state[3][0] ^= gf_mul[col[3]][0];
   // Column 2
   col[0] = state[0][1];
   col[1] = state[1][1];
   col[2] = state[2][1];
   col[3] = state[3][1];
   state[0][1] = gf_mul[col[0]][0];
   state[0][1] ^= gf_mul[col[1]][1];
   state[0][1] ^= col[2];
   state[0][1] ^= col[3];
   state[1][1] = col[0];
   state[1][1] ^= gf_mul[col[1]][0];
   state[1][1] ^= gf_mul[col[2]][1];
   state[1][1] ^= col[3];
   state[2][1] = col[0];
   state[2][1] ^= col[1];
   state[2][1] ^= gf_mul[col[2]][0];
   state[2][1] ^= gf_mul[col[3]][1];
   state[3][1] = gf_mul[col[0]][1];
   state[3][1] ^= col[1];
   state[3][1] ^= col[2];
   state[3][1] ^= gf_mul[col[3]][0];
   // Column 3
   col[0] = state[0][2];
   col[1] = state[1][2];
   col[2] = state[2][2];
   col[3] = state[3][2];
   state[0][2] = gf_mul[col[0]][0];
   state[0][2] ^= gf_mul[col[1]][1];
   state[0][2] ^= col[2];
   state[0][2] ^= col[3];
   state[1][2] = col[0];
   state[1][2] ^= gf_mul[col[1]][0];
   state[1][2] ^= gf_mul[col[2]][1];
   state[1][2] ^= col[3];
   state[2][2] = col[0];
   state[2][2] ^= col[1];
   state[2][2] ^= gf_mul[col[2]][0];
   state[2][2] ^= gf_mul[col[3]][1];
   state[3][2] = gf_mul[col[0]][1];
   state[3][2] ^= col[1];
   state[3][2] ^= col[2];
   state[3][2] ^= gf_mul[col[3]][0];
   // Column 4
   col[0] = state[0][3];
   col[1] = state[1][3];
   col[2] = state[2][3];
   col[3] = state[3][3];
   state[0][3] = gf_mul[col[0]][0];
   state[0][3] ^= gf_mul[col[1]][1];
   state[0][3] ^= col[2];
   state[0][3] ^= col[3];
   state[1][3] = col[0];
   state[1][3] ^= gf_mul[col[1]][0];
   state[1][3] ^= gf_mul[col[2]][1];
   state[1][3] ^= col[3];
   state[2][3] = col[0];
   state[2][3] ^= col[1];
   state[2][3] ^= gf_mul[col[2]][0];
   state[2][3] ^= gf_mul[col[3]][1];
   state[3][3] = gf_mul[col[0]][1];
   state[3][3] ^= col[1];
   state[3][3] ^= col[2];
   state[3][3] ^= gf_mul[col[3]][0];
}

void InvMixColumns(uchar state[][4])
{
   // int idx;
   uchar col[4]; //,t;
   // Column 1
   col[0] = state[0][0];
   col[1] = state[1][0];
   col[2] = state[2][0];
   col[3] = state[3][0];
   state[0][0] = gf_mul[col[0]][5];
   state[0][0] ^= gf_mul[col[1]][3];
   state[0][0] ^= gf_mul[col[2]][4];
   state[0][0] ^= gf_mul[col[3]][2];
   state[1][0] = gf_mul[col[0]][2];
   state[1][0] ^= gf_mul[col[1]][5];
   state[1][0] ^= gf_mul[col[2]][3];
   state[1][0] ^= gf_mul[col[3]][4];
   state[2][0] = gf_mul[col[0]][4];
   state[2][0] ^= gf_mul[col[1]][2];
   state[2][0] ^= gf_mul[col[2]][5];
   state[2][0] ^= gf_mul[col[3]][3];
   state[3][0] = gf_mul[col[0]][3];
   state[3][0] ^= gf_mul[col[1]][4];
   state[3][0] ^= gf_mul[col[2]][2];
   state[3][0] ^= gf_mul[col[3]][5];
   // Column 2
   col[0] = state[0][1];
   col[1] = state[1][1];
   col[2] = state[2][1];
   col[3] = state[3][1];
   state[0][1] = gf_mul[col[0]][5];
   state[0][1] ^= gf_mul[col[1]][3];
   state[0][1] ^= gf_mul[col[2]][4];
   state[0][1] ^= gf_mul[col[3]][2];
   state[1][1] = gf_mul[col[0]][2];
   state[1][1] ^= gf_mul[col[1]][5];
   state[1][1] ^= gf_mul[col[2]][3];
   state[1][1] ^= gf_mul[col[3]][4];
   state[2][1] = gf_mul[col[0]][4];
   state[2][1] ^= gf_mul[col[1]][2];
   state[2][1] ^= gf_mul[col[2]][5];
   state[2][1] ^= gf_mul[col[3]][3];
   state[3][1] = gf_mul[col[0]][3];
   state[3][1] ^= gf_mul[col[1]][4];
   state[3][1] ^= gf_mul[col[2]][2];
   state[3][1] ^= gf_mul[col[3]][5];
   // Column 3
   col[0] = state[0][2];
   col[1] = state[1][2];
   col[2] = state[2][2];
   col[3] = state[3][2];
   state[0][2] = gf_mul[col[0]][5];
   state[0][2] ^= gf_mul[col[1]][3];
   state[0][2] ^= gf_mul[col[2]][4];
   state[0][2] ^= gf_mul[col[3]][2];
   state[1][2] = gf_mul[col[0]][2];
   state[1][2] ^= gf_mul[col[1]][5];
   state[1][2] ^= gf_mul[col[2]][3];
   state[1][2] ^= gf_mul[col[3]][4];
   state[2][2] = gf_mul[col[0]][4];
   state[2][2] ^= gf_mul[col[1]][2];
   state[2][2] ^= gf_mul[col[2]][5];
   state[2][2] ^= gf_mul[col[3]][3];
   state[3][2] = gf_mul[col[0]][3];
   state[3][2] ^= gf_mul[col[1]][4];
   state[3][2] ^= gf_mul[col[2]][2];
   state[3][2] ^= gf_mul[col[3]][5];
   // Column 4
   col[0] = state[0][3];
   col[1] = state[1][3];
   col[2] = state[2][3];
   col[3] = state[3][3];
   state[0][3] = gf_mul[col[0]][5];
   state[0][3] ^= gf_mul[col[1]][3];
   state[0][3] ^= gf_mul[col[2]][4];
   state[0][3] ^= gf_mul[col[3]][2];
   state[1][3] = gf_mul[col[0]][2];
   state[1][3] ^= gf_mul[col[1]][5];
   state[1][3] ^= gf_mul[col[2]][3];
   state[1][3] ^= gf_mul[col[3]][4];
   state[2][3] = gf_mul[col[0]][4];
   state[2][3] ^= gf_mul[col[1]][2];
   state[2][3] ^= gf_mul[col[2]][5];
   state[2][3] ^= gf_mul[col[3]][3];
   state[3][3] = gf_mul[col[0]][3];
   state[3][3] ^= gf_mul[col[1]][4];
   state[3][3] ^= gf_mul[col[2]][2];
   state[3][3] ^= gf_mul[col[3]][5];
}

/********************
** DEBUGGING FUNCTIONS
********************/

// This prints the "state" grid as a linear hex string
void printstate(uchar state[][4])
{
   int idx,idx2;
   for (idx=0; idx < 4; idx++)
    {
      for (idx2=0; idx2 < 4; idx2++)
         printf("%02x ",state[idx][idx2]);
      printf("\n\r");
    }
    printf("\n\r");
   //puts("");
}

void printfaultstate(uchar state[][4], uchar fltstate[][4])
{
   int idx,idx2;
   for (idx=0; idx < 4; idx++)
    {
      for (idx2=0; idx2 < 4; idx2++)
         printf("%02x ",state[idx][idx2]);
      printf("      ");
      for (idx2=0; idx2 < 4; idx2++)
         printf("%02x ",fltstate[idx][idx2]);
      printf("        ");
      for (idx2=0; idx2 < 4; idx2++)
        if (state[idx][idx2]^fltstate[idx][idx2])
           printf("%02x ", state[idx][idx2]^fltstate[idx][idx2] );
        else
            printf(" . ");
      printf("\n\r");
    }
    printf("\n\r");
   //puts("");
}

// This prints the key (4 consecutive ints) used for a given round as a linear hex string.
void print_rnd_key(uint key[])
{
   int idx;
   for (idx=0; idx < 4; idx++)
      printf("%08x",(unsigned int) key[idx]);
   puts("");
}

/********************
** AES (En/De)Crypt
********************/

// "in" is the block of 16 sequencial bytes that is to be encrypted. "out" is the encrypted
// sequencial output. "key" is an array consisting of the KEY value that was generated
// using KeySchedule() previously. "keysize" MUST be 128, 192, 256 in size.
void aes_encrypt(uchar in[], uchar out[], uchar key[] )
{
   uchar state[4][4], roundkey[4][4];
   uchar rc = 0x01;
   int i;
   // Copy input array (should be 16 bytes long) to a matrix (sequential bytes are ordered
   // by row, not col) called "state" for processing.
   // *** Implementation note: The official AES documentation references the state by
   // column, then row. Accessing an element in C requires row then column. Thus, all state
   // references in AES must have the column and row indexes reversed for C implementation.
   state[0][0] = in[0];
   state[1][0] = in[1];
   state[2][0] = in[2];
   state[3][0] = in[3];
   state[0][1] = in[4];
   state[1][1] = in[5];
   state[2][1] = in[6];
   state[3][1] = in[7];
   state[0][2] = in[8];
   state[1][2] = in[9];
   state[2][2] = in[10];
   state[3][2] = in[11];
   state[0][3] = in[12];
   state[1][3] = in[13];
   state[2][3] = in[14];
   state[3][3] = in[15];
   Input2State( roundkey, key );

   // Perform the necessary number of rounds. The round key is added first.
   // The last round does not perform the MixColumns step.
   AddKey( state, roundkey ); next_key( roundkey, &rc );
   for (i=0; i<9; i++)
   {
      SubBytes(state); ShiftRows(state); MixColumns(state); AddKey( state, roundkey ); next_key( roundkey, &rc );
   }
   SubBytes(state); ShiftRows(state); AddKey( state, roundkey );

   // Copy the state to the output array
   out[0] = state[0][0];
   out[1] = state[1][0];
   out[2] = state[2][0];
   out[3] = state[3][0];
   out[4] = state[0][1];
   out[5] = state[1][1];
   out[6] = state[2][1];
   out[7] = state[3][1];
   out[8] = state[0][2];
   out[9] = state[1][2];
   out[10] = state[2][2];
   out[11] = state[3][2];
   out[12] = state[0][3];
   out[13] = state[1][3];
   out[14] = state[2][3];
   out[15] = state[3][3];
}

void aes_decrypt(uchar in[], uchar out[], uchar key[])
{
   uchar state[4][4], roundkey[4][4];
   uchar rc = 0x01;
   int i;
   // Copy the input to the state.
   state[0][0] = in[0];
   state[1][0] = in[1];
   state[2][0] = in[2];
   state[3][0] = in[3];
   state[0][1] = in[4];
   state[1][1] = in[5];
   state[2][1] = in[6];
   state[3][1] = in[7];
   state[0][2] = in[8];
   state[1][2] = in[9];
   state[2][2] = in[10];
   state[3][2] = in[11];
   state[0][3] = in[12];
   state[1][3] = in[13];
   state[2][3] = in[14];
   state[3][3] = in[15];
   Input2State( roundkey, key );

   // Perform the necessary number of rounds. The round key is added first.
   // The last round does not perform the MixColumns step.
   for (i=0; i<10; i++)
      next_key( roundkey, &rc );


   AddKey( state, roundkey );
   InvShiftRows(state);
   InvSubBytes(state);

   for (i=0; i<10; i++)
   {
      prev_key( roundkey, &rc );
      AddKey( state, roundkey );
      InvMixColumns(state);
      InvShiftRows(state);
      InvSubBytes(state);
   }
   prev_key( roundkey, &rc );
   AddKey( state, roundkey );


   // Copy the state to the output array
   out[0] = state[0][0];
   out[1] = state[1][0];
   out[2] = state[2][0];
   out[3] = state[3][0];
   out[4] = state[0][1];
   out[5] = state[1][1];
   out[6] = state[2][1];
   out[7] = state[3][1];
   out[8] = state[0][2];
   out[9] = state[1][2];
   out[10] = state[2][2];
   out[11] = state[3][2];
   out[12] = state[0][3];
   out[13] = state[1][3];
   out[14] = state[2][3];
   out[15] = state[3][3];
}


void aes_fltdecrypt(uchar in[], uchar fin[], uchar key[])
{
   uchar state[4][4], fst[4][4], roundkey[4][4];
   uchar rc = 0x01;
   int i;
   // Copy the input to the state.
   state[0][0] = in[0];
   state[1][0] = in[1];
   state[2][0] = in[2];
   state[3][0] = in[3];
   state[0][1] = in[4];
   state[1][1] = in[5];
   state[2][1] = in[6];
   state[3][1] = in[7];
   state[0][2] = in[8];
   state[1][2] = in[9];
   state[2][2] = in[10];
   state[3][2] = in[11];
   state[0][3] = in[12];
   state[1][3] = in[13];
   state[2][3] = in[14];
   state[3][3] = in[15];

   fst[0][0] = fin[0];
   fst[1][0] = fin[1];
   fst[2][0] = fin[2];
   fst[3][0] = fin[3];
   fst[0][1] = fin[4];
   fst[1][1] = fin[5];
   fst[2][1] = fin[6];
   fst[3][1] = fin[7];
   fst[0][2] = fin[8];
   fst[1][2] = fin[9];
   fst[2][2] = fin[10];
   fst[3][2] = fin[11];
   fst[0][3] = fin[12];
   fst[1][3] = fin[13];
   fst[2][3] = fin[14];
   fst[3][3] = fin[15];

   Input2State( roundkey, key );

   // Perform the necessary number of rounds. The round key is added first.
   // The last round does not perform the MixColumns step.
   for (i=0; i<10; i++)
      next_key( roundkey, &rc );


   printf("OUT\n\r");
   printfaultstate(state, fst); printf("\n\r");
   // Perform the necessary number of rounds. The round key is added first.
   // The last round does not perform the MixColumns step.

   printf("R10\n\r");
   printf("             AK\n\r");
   AddKey( state, roundkey );       AddKey( fst, roundkey );        printfaultstate(state, fst);
   printf("             SR\n\r");
   InvShiftRows(state);             InvShiftRows(fst);              printfaultstate(state, fst);
   printf("             SB\n\r");
   InvSubBytes(state);              InvSubBytes(fst);               printfaultstate(state, fst);
   printf("            input\n\r");
   printf("\n\r");

   printf("R9\n\r");
   printf("             AK\n\r");
   prev_key( roundkey, &rc );
   AddKey( state, roundkey );       AddKey( fst, roundkey );        printfaultstate(state, fst);
   printf("             MC\n\r");
   InvMixColumns(state);            InvMixColumns(fst);             printfaultstate(state, fst);
   printf("             SR\n\r");
   InvShiftRows(state);             InvShiftRows(fst);              printfaultstate(state, fst);
   printf("             SB\n\r");
   InvSubBytes(state);              InvSubBytes(fst);               printfaultstate(state, fst);
   printf("            input\n\r");
   printf("\n\r");

   printf("R8\n\r");
   printf("             AK\n\r");
   prev_key( roundkey, &rc );
   AddKey( state, roundkey );       AddKey( fst, roundkey );        printfaultstate(state, fst);
   printf("             MC\n\r");
   InvMixColumns(state);            InvMixColumns(fst);             printfaultstate(state, fst);
/*
   printf("             SR\n\r");
   InvShiftRows(state);             InvShiftRows(fst);              printfaultstate(state, fst);
   printf("             SB\n\r");
   InvSubBytes(state);              InvSubBytes(fst);               printfaultstate(state, fst);
   printf("            input\n\r");
   printf("\n\r");
// */
/*
   AddRoundKey(state,&key[28]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[24]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[20]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[16]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[12]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[8]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[4]);InvMixColumns(state);
   InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[0]);

   // Copy the state to the output array
   out[0] = state[0][0];
   out[1] = state[1][0];
   out[2] = state[2][0];
   out[3] = state[3][0];
   out[4] = state[0][1];
   out[5] = state[1][1];
   out[6] = state[2][1];
   out[7] = state[3][1];
   out[8] = state[0][2];
   out[9] = state[1][2];
   out[10] = state[2][2];
   out[11] = state[3][2];
   out[12] = state[0][3];
   out[13] = state[1][3];
   out[14] = state[2][3];
   out[15] = state[3][3];
   */
}


void Hex2Char(char hex[33], uchar Char[16] )
{
    int i=0;

    for ( i=0; i<16; i++ )
    {
        if (hex[2*i]>='0' && hex[2*i]<='9') Char[i]=(hex[2*i]-'0')*16;
        if (hex[2*i]>='a' && hex[2*i]<='f') Char[i]=(hex[2*i]-'a'+10)*16;
        if (hex[2*i]>='A' && hex[2*i]<='F') Char[i]=(hex[2*i]-'A'+10)*16;

        if (hex[2*i+1]>='0' && hex[2*i+1]<='9') Char[i]+=(hex[2*i+1]-'0');
        if (hex[2*i+1]>='a' && hex[2*i+1]<='f') Char[i]+=(hex[2*i+1]-'a'+10);
        if (hex[2*i+1]>='A' && hex[2*i+1]<='F') Char[i]+=(hex[2*i+1]-'A'+10);
    }
}


int Count_Faulty_Bytes( uchar flt_in[][4] )
{
   int count;
   int i,j;
   for ( i=0, count=0; i<4; i++ )
      for ( j=0; j<4; j++ )
         if ( flt_in[i][j] ) count++;
   return count;
}

int count_bits( uchar byte )
{
   int count = 0;
   int i;
   for ( i=0; i<8; i++ )
   {
      if ( byte & 1 ) count++;
      byte = byte >> 1;
   }
   return count;
}

int Count_Faulty_Bits( uchar flt_in[][4] )
{
   int count;
   int i,j;
   for ( i=0, count=0; i<4; i++ )
      for ( j=0; j<4; j++ )
         count += count_bits( flt_in[i][j] );
   return count;
}

void ComputeStateFault( uchar flt[][4], uchar st[][4], uchar fst[][4] )
{
   int i, j;
   for ( i=0; i<4; i++ )
      for ( j=0; j<4; j++ )
         flt[i][j] = st[i][j] ^ fst[i][j];
}

void CreateFaultMask( unsigned char mask[], uchar flt )
{
   int i;
   for ( i=7; i>=0; i-- )
   {
      mask[i] = ( ( flt & 1 ) ? '1' : '.' );
      flt = flt >> 1;
   }
   mask[8] = '\0';
}

void ConvertFault( unsigned char* dest, uchar flt[][4] )
{
   int i, j;
   for ( i=0; i<4; i++ )
      for ( j=0; j<4; j++ )
         dest[i+4*j] = flt[i][j];
   dest[16] = '\0';
}

int IsFaultHorizontal( uchar in[], uchar fin[] )
{
   int row, j, i;
   int tmp = 0;
   uchar state[4][4], fst[4][4], flt[4][4];

   Input2State( state, in );
   Input2State( fst, fin );
   ComputeStateFault( flt, state, fst );
   if ( Count_Faulty_Bytes( flt )==1 )
      return 0;
   else
      // Count how many identical byte errors for each line
      // if greater than 0, probablty a horizontal fault, thus a key fault
      for ( row=0; row<4; row++ )
         // for each line, compare all pairs:
         for ( i=0; i<3; i++ )
            for ( j=i+1; j<4; j++ )
               if ( flt[row][i]==flt[row][j] ) tmp++;
   return tmp;
}


int CheckPatternRow( uchar fault[][4], char* row_pattern )
{
   int i=0;
   int row, col;
   for ( row=0; row<4; row++ )
      for ( col=0; col<4; col++, i++ )
         if ( (row_pattern[i]=='0' || row_pattern[i]=='.') != (fault[row][col]=='\0') )
            return 0;
   return 1;
}


int CheckPatternCol( uchar fault[][4], char* col_pattern )
{
   int i=0;
   int row, col;
   for ( col=0; col<4; col++ )
      for ( row=0; row<4; row++, i++ )
         if ( (col_pattern[i]=='0' || col_pattern[i]=='.') != (fault[row][col]=='\0') )
            return 0;
   return 1;
}


// Check if fault=value may have occurred at round x, in roundkey y (->AK if ==x ; ->Key generation if ==x-1) at byte z
int Check_KS_Fault_Rx_RKy_Bz( int x, int y, int z, uchar value, uchar golden[], uchar faulty[], uchar key[], uchar *correct_byte )
{
   int round;
   int row, col;
   uchar rc = 0x01;
   uchar roundkey[4][4];
   uchar text[4][4], fst[4][4];

   // Unroll & Get Last round key
   Input2State( roundkey, key );
   for ( round=0; round <10; round++ )
      next_key( roundkey, &rc );
   // Rewind to round x
   Input2State( text, golden );
   for ( round=10; ; round-- )
   {
      AddKey( text, roundkey );
      prev_key( roundkey, &rc );
      if ( round==x ) break;
      if ( round!=10 ) InvMixColumns( text );
      InvShiftRows( text );
      InvSubBytes( text );
   }
   // Inject fault
   row = (15-z) & 0x00000003;
   col = ( (15-z) >> 2 ) & 0x00000003;
   if ( x==y ) // Fault on current round key (current roundkey is generated from a proper roundkey)
   {
      next_key( roundkey, &rc );
      *correct_byte = roundkey[row][col];
      roundkey[row][col] ^= value;
   }
   if ( y==x-1 ) // Fault on previous round key (current roundkey is generated from a faulty roundkey)
   {
      *correct_byte = roundkey[row][col];
      roundkey[row][col] ^= value;
      next_key( roundkey, &rc );
   }
   if ( y==x+1 )
   {
      *correct_byte = roundkey[row][col];
      next_key_F( roundkey, &rc, value<<8*(3-row) ); /***** check *****/
   }
   // Finish encryption
   while( round<10 )
   {
      AddKey( text, roundkey );
      next_key( roundkey, &rc );
      round++;
      SubBytes( text );
      ShiftRows( text );
      if ( round!=10 ) MixColumns( text );
   }
   AddKey( text, roundkey );
   // Compare and Validate
   Input2State( fst, faulty );
   for ( row=0; row<4; row++ )
      for ( col=0; col<4; col++ )
         if ( text[row][col]!=fst[row][col] )
            return 0;
   return 1;
}

int OnlyOneFaultyLine( uchar flt_st[][4] )
{
   int row, col, faulty_row;
   faulty_row = 0;
   for ( row=0; row<4; row++ )
      for ( col=0; col<4; col++ )
         if ( flt_st[row][col] )
         {
            if ( !faulty_row ) faulty_row=row+1;
            else if ( faulty_row!=row+1 ) return 0;
         }
   return faulty_row;
}

uchar MostCommonFault( uchar flt_st[][4] )
{
   int flt_occurrence[256];
   int row, col, max=0, i;

   memset( flt_occurrence, 0, 256 );
   for ( row=0; row<4; row++ )
      for ( col=0; col<4; col++ )
         flt_occurrence[ flt_st[row][col] ]++;
   for ( i=0; i<256; i++ )
      if ( flt_occurrence[i]>flt_occurrence[max] ) i=max;
   return flt_occurrence[max];
}

void Analyze_Fault( TFaultStructure *fault_data, uchar golden[], uchar faulty[], uchar key[], uchar errorcode[] )
{
   int i;
   int x, y , z;
   uchar val, correct_key_byte;
   int timing;
   uchar set_flt, reset_flt;
   int n_f_B, n_f_b;

   uchar golden_state[4][4], faulty_state[4][4];
   uchar fault[4][4];
   //uchar fault_pattern[17];

   Input2State( golden_state, golden );
   Input2State( faulty_state, faulty );
   ComputeStateFault( fault, golden_state, faulty_state );

   fault_data->Start_Num_ByteFaults = Count_Faulty_Bytes( fault );
   fault_data->Start_Num_BitFaults = Count_Faulty_Bits( fault );
   //memcpy( fault_data->Start_Fault_Matrix, fault_pattern, 17 );
   for ( i=0; i<16; i++ )
      fault_data->Start_Fault_Matrix[i] = golden[i] ^ faulty[i];
   fault_data->Start_Fault_Matrix[16] = '\0';


   x = y = z = n_f_B = n_f_b = -1;
   val = 0x00;
   timing = -1;


   /** To modify if the cause of injected fault include key scheduling)**/
   Analyze_Fault_DFA( fault_data, golden, faulty, key, errorcode );
   return;

   //**** SINGLE BYTE FAULT ***************************************************************************
   //**** DIAGONAL FAULT ******************************************************************************
   if ( ( fault_data->Start_Num_ByteFaults==1 ) ||
        CheckPatternRow( fault, ".W..X......Y..Z." ) ||
        CheckPatternRow( fault, "W......X..Y..Z.." ) ||
        CheckPatternRow( fault, "...W..X..Y..Z..." ) ||
        CheckPatternRow( fault, "..W..X..Y......Z" )
       )
   {
      Analyze_Fault_DFA( fault_data, golden, faulty, key, errorcode );
      return;
   }
   //**** 2 FAULTY BYTES ******************************************************************************
   // R1 : ..AB
   if ( CheckPatternRow( fault, "..XY............" ) )
   {
      val = fault[0][3];
      y = 9;
      z = 7;
      if ( fault[0][2] == fault[0][3] )
      {
         x = 10;
         timing = R10_RK9;
      }
      else
      {
         x = 9;
         timing = R9_RK9;
      }
   }
   // R1 : AB..
   // R2 : AB..
   // R2 : ..AB
   if ( CheckPatternRow( fault, "......XX........" ) )
   {
      if ( fault[1][2] == fault[1][3] )
      {
         x = 10;
         y = 9;
         z = 6;
         val = fault[1][2];
         timing = R10_RK9;
      }
   }
   // R3 : ..AB
   if ( CheckPatternRow( fault, "..........XX...." ) )
   {
      if ( fault[2][2] == fault[2][3] )
      {
         x = 10;
         y = 9;
         z = 5;
         val = fault[2][2];
         timing = R10_RK9;
      }
   }
   // R4 : ..AB
   if ( CheckPatternRow( fault, "..............XY" ) )
   {
      val = fault[3][2];
      y = 9;
      z = 4;
      if ( fault[3][2] == fault[3][3] )
      {
         x = 10;
         timing = R10_RK9;
      }
      else
      {
         x = 9;
         timing = R9_RK9;
      }
   }
   //**** 3 FAULTY BYTES ******************************************************************************
   // R1: .AAA
   if ( CheckPatternRow( fault, ".XXX............" ) )
   {
      if ( (fault[0][2] == fault[0][3]) && (fault[0][3] == fault[0][1]) )
      {
         x = 10;
         y = 9;
         z = 11;
         val = fault[0][3];
         timing = R10_RK9;
      }
   }
   // R2: .AAA
   if ( CheckPatternRow( fault, ".....XXX........" ) )
   {
      if ( (fault[1][2] == fault[1][3]) && (fault[1][3] == fault[1][1]) )
      {
         x = 10;
         y = 9;
         z = 10;
         val = fault[1][3];
         timing = R10_RK9;
      }
   }
   // R3: .AAA
   if ( CheckPatternRow( fault, ".........XXX...." ) )
   {
      if ( (fault[2][2] == fault[2][3]) && (fault[2][3] == fault[2][1]) )
      {
         x = 10;
         y = 9;
         z = 9;
         val = fault[2][3];
         timing = R10_RK9;
      }
   }
   // R4: .AAA
   if ( CheckPatternRow( fault, ".............XXX" ) )
   {
      if ( (fault[3][2] == fault[3][3]) && (fault[3][3] == fault[3][1]) )
      {
         x = 10;
         y = 9;
         z = 8;
         val = fault[3][3];
         timing = R10_RK9;
      }
   }
   // R4: .ABA
   if ( CheckPatternRow( fault, ".............XYX" ) )
   {
      if ( (fault[3][2] != fault[3][1]) && (fault[3][3] == fault[3][1]) )
      {
         x = 9;
         y = 9;
         z = 8;
         val = fault[3][3];
         timing = R9_RK9;
      }
   }
   // R3: .AAB
   if ( CheckPatternRow( fault, ".........XXY...." ) )
   {
      if ( (fault[2][2] == fault[2][1]) && (fault[2][3] != fault[2][1]) )
      {
         x = 9;
         y = 9;
         z = 9;
         val = fault[2][1];
         timing = R9_RK9;
      }
   }
   // R3: A.BC
   if ( CheckPatternRow( fault, "........Y.XX...." ) )
   {
      if ( (fault[2][2] == fault[2][3]) && (fault[2][0] != fault[2][2]) )
      {
         x = 9;
         y = 9;
         z = 5;
         val = fault[2][2];
         timing = R9_RK9;
      }
   }
   // R1: .BAA
   if ( CheckPatternRow( fault, ".YXX............" ) )
   {
      if ( (fault[0][2] == fault[0][3]) && (fault[0][3] != fault[0][1]) )
      {
         x = 9;
         y = 9;
         z = 11;
         val = fault[0][3];
         timing = R9_RK9;
      }
   }
   // R2: .BAA
   if ( CheckPatternRow( fault, ".....YXX........" ) )
   {
      if ( (fault[1][2] == fault[1][3]) && (fault[1][3] != fault[1][1]) )
      {
         x = 9;
         y = 9;
         z = 6;
         val = fault[1][3];
         timing = R9_RK9;
      }
   }
   //**** 4 FAULTY BYTES ******************************************************************************
   // R1: AAAA
   if ( CheckPatternRow( fault, "XXXX............" ) ) // then
   {
      if ( (fault[0][0] == fault[0][1]) && (fault[0][1] == fault[0][2]) && (fault[0][2] == fault[0][3]) )
      {
         if ( count_bits( fault[0][0] )==1 ) // what about multiple bit errors ?
         {
            x = 10;
            y = 9;
            z = 15;
            val = fault[0][0];
            timing = R10_RK9;
         }
         else // fault at Key Sbox Input ?
         {
            x = 10;
            y = 11;
            z = 3;
            val = 1;
            while ( !val && !Check_KS_Fault_Rx_RKy_Bz(x, y, z, val, golden, faulty, key, &correct_key_byte) )
                val++;
            timing = R10_KSB;
         }
      }
   }
   // R2: AAAA
   if ( CheckPatternRow( fault, "....XXXX........" ) )
   {
      if ( (fault[1][0] == fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] == fault[1][3]) )
      {
         if ( count_bits( fault[1][0] )==1 ) // what about multiple bit errors ?
         {
            x = 10;
            y = 9;
            z = 14;
            val = fault[1][0];
            timing = R10_RK9;
         }
         else // fault at Key Sbox Input ?
         {
            x = 10;
            y = 11;
            z = 2;
            val = 1;
            while ( !val && !Check_KS_Fault_Rx_RKy_Bz(x, y, z, val, golden, faulty, key, &correct_key_byte) )
                val++;
            timing = R10_KSB;
         }
      }
   }
   // R3: AAAA
   if ( CheckPatternRow( fault, "........XXXX...." ) )
   {
      if ( (fault[2][0] == fault[2][1]) && (fault[2][1] == fault[2][2]) && (fault[2][2] == fault[2][3]) )
      {
         if ( count_bits( fault[2][0] )==1 ) // what about multiple bit errors ?
         {
            x = 10;
            y = 9;
            z = 13;
            val = fault[2][0];
            timing = R10_RK9;
         }
         else // fault at Key Sbox Input ?
         {
            x = 10;
            y = 11;
            z = 1;
            val = 1;
            while ( !val && !Check_KS_Fault_Rx_RKy_Bz(x, y, z, val, golden, faulty, key, &correct_key_byte) )
                val++;
            timing = R10_KSB;
         }
      }
   }
   // R4: AAAA
   if ( CheckPatternRow( fault, "............XXXX" ) )
   {
      if ( (fault[3][0] == fault[3][1]) && (fault[3][1] == fault[3][2]) && (fault[3][2] == fault[3][3]) )
      {
         if ( count_bits( fault[3][0] )==1 ) // what about multiple bit errors ?
         {
            x = 10;
            y = 9;
            z = 12;
            val = fault[3][0];
            timing = R10_RK9;
         }
         else // fault at Key Sbox Input ?
         {
            x = 10;
            y = 11;
            z = 0;
            val = 1;
            while ( !val && !Check_KS_Fault_Rx_RKy_Bz(x, y, z, val, golden, faulty, key, &correct_key_byte) )
                val++;
            timing = R10_KSB;
         }
      }
   }
   // R1: BAAA
   if ( CheckPatternRow( fault, "YXXX............" ) )
   {
      if ( (fault[0][0] != fault[0][1]) && (fault[0][1] == fault[0][2]) && (fault[0][2] == fault[0][3]) )
      {
         x = 9;
         y = 9;
         z = 15;
         val = fault[0][1];
         timing = R9_RK9;
      }
   }
   // R2: BAAA
   if ( CheckPatternRow( fault, "....YXXX........" ) )
   {
      if ( (fault[1][0] != fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] == fault[1][3]) )
      {
         x = 9;
         y = 9;
         z = 10;
         val = fault[1][1];
         timing = R9_RK9;
      }
   }
   // R2: AAAB
   if ( CheckPatternRow( fault, "....XXXY........" ) )
   {
      if ( (fault[1][0] == fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] != fault[1][3]) )
      {
         x = 9;
         y = 9;
         z = 14;
         val = fault[1][1];
         timing = R9_RK9;
      }
   }
   // R3: AABA
   if ( CheckPatternRow( fault, "........XXYX...." ) )
   {
      if ( (fault[2][0] == fault[2][1]) && (fault[2][1] != fault[2][2]) && (fault[2][1] == fault[2][3]) )
      {
         x = 9;
         y = 9;
         z = 13;
         val = fault[2][1];
         timing = R9_RK9;
      }
   }
   // R4: ABAA
   if ( CheckPatternRow( fault, "............XYXX" ) )
   {
      if ( (fault[3][0] != fault[3][1]) && (fault[3][0] == fault[3][2]) && (fault[3][2] == fault[3][3]) )
      {
         x = 9;
         y = 9;
         z = 12;
         val = fault[3][0];
         timing = R9_RK9;
      }
   }
   //**** 5 FAULTY BYTES ******************************************************************************
   // R1: ...A; R4: BBBB
   if ( CheckPatternRow( fault, "...Y........XXXX" ) )
   {
      if ( (fault[3][0] == fault[3][1]) && (fault[3][1] == fault[3][2]) && (fault[3][2] == fault[3][3]) )
      {
         if ( count_bits( fault[0][3] )==1 ) // what about multiple bit errors ?
         {
            x = 10;
            y = 9;
            z = 3;
            val = fault[0][3];
            timing = R10_RK9;
         }
         else
         {
            x = 9;
            y = 9;
            z = 3;
            val = 1;
            while ( !val && !(Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
               val++;
            timing = R9_RK9;
            if ( !val )
            {
               x = 10;
               y = 9;
               z = 3;
               val = 1;
               while ( !val && !(Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
                  val++;
               timing = R10_RK9;
            }
         }
      }
   }
   //**** 6 FAULTY BYTES ******************************************************************************
   // R1: ..AC; R4: BBBB
   if ( CheckPatternRow( fault, "..WY........XXXX" ) )
   {
      if ( (fault[3][0] == fault[3][1]) && (fault[3][1] == fault[3][2]) && (fault[3][2] == fault[3][3]) )
      {
         x = 9;
         y = 8;
         z = 7;
         val = 1;
         while ( !val && !( Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
      }
   }
   // R2: AAAA R3: .B.C
   if ( CheckPatternRow( fault, "....XXXX.Y.Z...." ) )
   {
      if ( (fault[1][0] == fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] == fault[1][3]) )
         if ( count_bits( fault[2][3] )==1 ) // what about multiple bit errors ?
         {
            x = 9;
            y = 9;
            z = 1;
            val = fault[2][3];
            timing = R9_RK9;
         }
   }
   // R1: AAAA; R2: .BC.
   if ( CheckPatternRow( fault, "XXXX.YZ........." ) )
   {
      if ( (fault[0][0] == fault[0][1]) && (fault[0][1] == fault[0][2]) && (fault[0][2] == fault[0][3]) )
      {
         x = 9;
         y = 8;
         z = 6;
         val = 1;
         while ( !val && !( Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
      }
   }
   // R1: AAAA; R2: ..BC
   if ( CheckPatternRow( fault, "XXXX..YZ........" ) )
   {
      if ( (fault[0][0] == fault[0][1]) && (fault[0][1] == fault[0][2]) && (fault[0][2] == fault[0][3]) )
      {
         x = 9;
         y = 9;
         z = 2;
         val = fault[1][3];
         timing = R9_RK9;
      }
   }
   // R3: AAAA; R4: B..C
   if ( CheckPatternRow( fault, "........XXXXY..Z" ) )
   {
      if ( (fault[2][0] == fault[2][1]) && (fault[2][1] == fault[2][2]) && (fault[2][2] == fault[2][3]) )
         if ( count_bits( fault[3][3] )==1 ) // what about multiple bit errors ?
         {
            x = 9;
            y = 9;
            z = 0;
            val = fault[3][3];
            timing = R9_RK9;
         }
   }
   //**** 7 FAULTY BYTES ******************************************************************************
   // R3: AAAA; R4: B.CD
   if ( CheckPatternRow( fault, "........XXXXW.YZ" ) )
   {
      if ( (fault[2][0] == fault[2][1]) && (fault[2][1] == fault[2][2]) && (fault[2][2] == fault[2][3]) )
      {
         x = 9;
         y = 8;
         z = 4;
         val = fault[3][2];
         timing = R9_RK8;
      }
   }
   // R2: AAAA; R3: BC.D
   if ( CheckPatternRow( fault, "....XXXXWY.Z...." ) )
   {
      if ( (fault[1][0] == fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] == fault[1][3]) )
      {
         x = 9;
         y = 8;
         z = 9;
         val = 1;
         while ( !val && !( Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
      }
   }
   // R2: AAAA; R3: BCD.
   if ( CheckPatternRow( fault, "....XXXXWY.Z...." ) )
   {
      if ( (fault[1][0] == fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] == fault[1][3]) )
         if ( count_bits( fault[2][2] )==1 ) // what about multiple bit errors ?
         {
            x = 9;
            y = 8;
            z = 5;
            val = fault[2][2];
            timing = R9_RK8;
         }
   }
   // R1: .BCD; R4: AAAA
   if ( CheckPatternRow( fault, ".WYZ........XXXX" ) )
   {
      if ( (fault[3][0] == fault[3][1]) && (fault[3][1] == fault[3][2]) && (fault[3][2] == fault[3][3]) )
      {
         x = 9;
         y = 8;
         z = 11;
         val = 1;
         while ( !val && !( Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
      }
   }
   //**** 8 FAULTY BYTES ******************************************************************************
   // R3: AAAA; R4: BCDE
   if ( CheckPatternRow( fault, "........XXXXWTYZ" ) ) // then
   {
      if ( (fault[2][0] == fault[2][1]) && (fault[2][1] == fault[2][2]) && (fault[2][2] == fault[2][3]) )
      {
         x = 9;
         y = 8;
         z = 12;
         val = 1;
         while ( val && (!Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
         if ( !val )
         {
             x = 9;
             y = 10;
             z = 0;
             val = 1;
             while ( val && (!Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
                val++;
             timing = R9_KSB;
         }
      }
   }
   // R2: AAAA; R3: BCDE
   if ( CheckPatternRow( fault, "....XXXXWYTZ...." ) )
   {
      if ( (fault[1][0] == fault[1][1]) && (fault[1][1] == fault[1][2]) && (fault[1][2] == fault[1][3]) )
      {
         x = 9;
         y = 8;
         z = 13;
         val = 1;
         while ( val && (!Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
         if ( !val )
         {
             x = 9;
             y = 10;
             z = 1;
             val = 1;
             while ( val && (!Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
                val++;
             timing = R9_KSB;
         }
      }
   }
   // R1: AAAA; R2: BCDE
   if ( CheckPatternRow( fault, "XXXXWTYZ........" ) )
   {
      if ( (fault[0][0] == fault[0][1]) && (fault[0][1] == fault[0][2]) && (fault[0][2] == fault[0][3]) )
      {
         x = 9;
         y = 8;
         z = 14;
         val = 1;
         while ( val && (! Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte )) )
            val++;
         timing = R9_RK8;
         if ( !val )
         {
             x = 9;
             y = 10;
             z = 2;
             val = 1;
             while ( val && (!Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
                val++;
             timing = R9_KSB;
         }
      }
   }
   // R4: AAAA; R1: BCDE
   if ( CheckPatternRow( fault, "TWYZ........XXXX" ) )
   {
      if ( (fault[3][0] == fault[3][1]) && (fault[3][1] == fault[3][2]) && (fault[3][2] == fault[3][3]) )
      {
         x = 9;
         y = 8;
         z = 15;
         val = 1;
         while ( val && (! Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
            val++;
         timing = R9_RK8;
         if ( !val )
         {
             x = 9;
             y = 10;
             z = 3;
             val = 1;
             while ( val && (!Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) ) )
                val++;
             timing = R9_KSB;
         }
      }
   }


   if ( !val )
   {
      // printf( "." );
   }
   if ( x!=-1 )
      if ( Check_KS_Fault_Rx_RKy_Bz( x, y, z, val, golden, faulty, key, &correct_key_byte ) )
      {
         fault_data->Num_of_Byte_Faults = 1;
         fault_data->Num_of_Bit_Faults = count_bits( val );
         fault_data->Fault_Occurrence = timing;
         for ( i=0; i<16; i++ )
            fault_data->Faults_Value[i] = '\0';
         fault_data->Faults_Value[ z ] = val;
         fault_data->Fault_Position = z;
         fault_data->Fault_Value = val;
         fault_data->Num_of_BitFlips = count_bits( val );
         CreateFaultMask( fault_data->Fault_Mask, fault_data->Fault_Value );

         reset_flt = (correct_key_byte) & (fault_data->Fault_Value);
         CreateFaultMask( fault_data->Fault_Reset_Mask, reset_flt );
         fault_data->Num_of_BitResets = count_bits( reset_flt );
         set_flt = (~correct_key_byte) & (fault_data->Fault_Value);
         CreateFaultMask( fault_data->Fault_Set_Mask, set_flt );
         fault_data->Num_of_BitSets = count_bits( set_flt );
         return;
      }

   fault_data->Num_of_Byte_Faults = 99;
   fault_data->Num_of_Bit_Faults = 999;
   fault_data->Fault_Occurrence = -1;
   for ( i=0; i<16; i++ )
      fault_data->Faults_Value[i] = '\0';
   fault_data->Fault_Position = 99;
   fault_data->Fault_Value = 0;
   fault_data->Num_of_BitFlips = 0;
   CreateFaultMask( fault_data->Fault_Mask, fault_data->Fault_Value );

   reset_flt = (correct_key_byte) & (fault_data->Fault_Value);
   CreateFaultMask( fault_data->Fault_Reset_Mask, reset_flt );
   fault_data->Num_of_BitResets = count_bits( reset_flt );
   set_flt = (~correct_key_byte) & (fault_data->Fault_Value);
   CreateFaultMask( fault_data->Fault_Set_Mask, set_flt );
   fault_data->Num_of_BitSets = count_bits( set_flt );
}

int is_odd( int val )
{
   return ( val % 2 );
}

int is_even( int val )
{
   return ( 1-( val % 2 ) );
}

void print_4x4_char(uchar state[4][4]){
   for(int i=0;i<4;i++){
      for(int j=0;j<4;j++){
         printf("%2X  ",state[i][j]);
      }
      printf("\n");
   }
   printf("\n");
}

void Analyze_Fault_DFA( TFaultStructure *fault_data, uchar in[], uchar fin[], uchar secretkey[], uchar errorcode[] )
{
   uchar state[4][4], fst[4][4], flt[4][4], roundkey[4][4];
   uchar min_state[4][4], min_fst[4][4];
   uchar rc = 0x01;
   int r;
   int n_f_B, n_f_b, temp_B, temp_b;
   int line, col;
   uchar set_flt, reset_flt;
   enum Fault_Timing current_op, min_op;

   // Unroll key
   Input2State( roundkey, secretkey );
   for ( r=0; r<10; r++ )
      next_key( roundkey, &rc );

   // Copy the input to the state.
   Input2State( state, in );
//   state[0][0] = in[0];  state[0][1] = in[4];   state[0][2] = in[8];   state[0][3] = in[12];
//   state[1][0] = in[1];  state[1][1] = in[5];   state[1][2] = in[9];   state[1][3] = in[13];
//   state[2][0] = in[2];  state[2][1] = in[6];   state[2][2] = in[10];  state[2][3] = in[14];
//   state[3][0] = in[3];  state[3][1] = in[7];   state[3][2] = in[11];  state[3][3] = in[15];

   Input2State( fst, fin );
//   fst[0][0] = fin[0];   fst[1][0] = fin[1];   fst[2][0] = fin[2];   fst[3][0] = fin[3];
//   fst[0][1] = fin[4];   fst[1][1] = fin[5];   fst[2][1] = fin[6];   fst[3][1] = fin[7];
//   fst[0][2] = fin[8];   fst[1][2] = fin[9];   fst[2][2] = fin[10];  fst[3][2] = fin[11];
//   fst[0][3] = fin[12];  fst[1][3] = fin[13];  fst[2][3] = fin[14];  fst[3][3] = fin[15];

   current_op = R10_OUT;
   ComputeStateFault( flt, state, fst );
   int min_B = Count_Faulty_Bytes( flt );
   int min_b = Count_Faulty_Bits( flt );

   for( int ind=0; ind<4 ;ind++){
             memcpy(min_state[ind], state[ind], 4);
             memcpy(min_fst[ind], fst[ind], 4);
      }
   min_op=current_op;

   for ( r=10; r>0; r-- )
   {

      if ( r==10 )
      {
         fault_data->Start_Num_ByteFaults = min_B;
         fault_data->Start_Num_BitFaults = min_b;
         ConvertFault( fault_data->Start_Fault_Matrix, flt );
      }

      // AddRoundKey
      AddKey( state, roundkey );        AddKey( fst, roundkey );
      ComputeStateFault( flt, state, fst );
      temp_B = Count_Faulty_Bytes( flt );
      temp_b = Count_Faulty_Bits( flt );
      if ( ( temp_B < min_B ) || ( ( temp_b <= min_b ) && ( temp_B == min_B ) ) )
      {
         for( int ind=0; ind<4 ;ind++){
             memcpy(min_state[ind], state[ind], 4);
             memcpy(min_fst[ind], fst[ind], 4);
            }
          min_op=current_op;
          min_B=temp_B;
          min_b=temp_b;
         
      }     
      prev_key( roundkey, &rc );  
      current_op--;
     

      // MixColumns
      if ( r!=10 ) // MixCol not in last round
      {
         InvMixColumns(state);            InvMixColumns(fst);
         ComputeStateFault( flt, state, fst );
         temp_B = Count_Faulty_Bytes( flt );
         temp_b = Count_Faulty_Bits( flt );
         if ( ( temp_B < min_B ) || ( ( temp_b <= min_b ) && ( temp_B == min_B ) ) )
         {
            for( int ind=0; ind<4 ;ind++){
               memcpy(min_state[ind], state[ind], 4);
               memcpy(min_fst[ind], fst[ind], 4);
               }
            min_op=current_op;
            min_B=temp_B;
            min_b=temp_b;
            
         }     
      }
         current_op--;

      // ShiftRows
      InvShiftRows(state);             InvShiftRows(fst);
      
      ComputeStateFault( flt, state, fst );
      temp_B = Count_Faulty_Bytes( flt );
      temp_b = Count_Faulty_Bits( flt );
         if ( ( temp_B < min_B ) || ( ( temp_b <= min_b ) && ( temp_B == min_B ) ) )
         {
            for( int ind=0; ind<4 ;ind++){
               memcpy(min_state[ind], state[ind], 4);
               memcpy(min_fst[ind], fst[ind], 4);
               }
            min_op=current_op;
            min_B=temp_B;
            min_b=temp_b;
            
         }    
      
      current_op--;

      // SubBytes
      InvSubBytes(state);              InvSubBytes(fst);
      ComputeStateFault( flt, state, fst );
      temp_B = Count_Faulty_Bytes( flt );
      temp_b = Count_Faulty_Bits( flt );
      if ( ( temp_B < min_B ) || ( ( temp_b <= min_b ) && ( temp_B == min_B ) ) )
      {
         // if error code and error arity of best solution are incongruent, then save less-optimal solution
         // - check error code
         // - check arity (# of bits) of minimal fault (n_f_b)
         // - check arity of less-optimal solution (temp_b)
         // if minimal incongruent (xor) and less-optimal congruent, then save less-optimal solution (current one before rewinding 1 step)
         if ( ( ( is_even( n_f_b ) && strcmp(errorcode, "0000") ) || // even-order minimal fault, but detected
                ( is_odd( n_f_b ) && !strcmp(errorcode, "0000") ) ) && // odd-order minimal fault, not detected
              ( ( is_odd( temp_b ) && strcmp(errorcode, "0000") ) || // odd-order less-optimal fault, but detected
                ( is_even( temp_b ) && !strcmp(errorcode, "0000") ) )  // even-order less-optimal fault, not detected
            ) // check for alternate solution
         {
            Alt_IsSet = 1;
            ConvertFault( Alt_Faults_Value, flt );
               for ( col=0; col<4; col++ )
                  for ( line=0; line<4; line++ )
                     if ( flt[line][col] )
                     {
                        Alt_Fault_Position = 15-(4*col)-line;
                        Alt_Fault_Value = flt[line][col];
                        Alt_Num_of_BitFlips = count_bits( Alt_Fault_Value );
                     }
         }
         for( int ind=0; ind<4 ;ind++){
            memcpy(min_state[ind], state[ind], 4);
            memcpy(min_fst[ind], fst[ind], 4);
          }
            min_op=current_op;
            min_B=temp_B;
            min_b=temp_b;
            
      }

      current_op--;

   }
   fault_data->Fault_Occurrence = min_op;
   ComputeStateFault( flt, min_state, min_fst );
   print_4x4_char(flt);
   ConvertFault( fault_data->Faults_Value, flt );
   fault_data->Num_of_Byte_Faults = Count_Faulty_Bytes( flt );
   if(fault_data->Num_of_Byte_Faults==0)
      fault_data->Fault_Occurrence = 0;
   fault_data->Num_of_Bit_Faults = Count_Faulty_Bits( flt );
   for ( col=0; col<4; col++ )
      for ( line=0; line<4; line++ )
         if ( flt[line][col] )
         {
            fault_data->Fault_Position = 15-(4*col)-line;
            fault_data->Fault_Value = flt[line][col];
            fault_data->Num_of_BitFlips = count_bits( fault_data->Fault_Value );
            reset_flt = state[line][col] & flt[line][col];
            set_flt = fst[line][col] & flt[line][col];
            CreateFaultMask( fault_data->Fault_Mask, fault_data->Fault_Value );
            CreateFaultMask( fault_data->Fault_Reset_Mask, reset_flt );
            fault_data->Num_of_BitResets = count_bits( reset_flt );
            CreateFaultMask( fault_data->Fault_Set_Mask, set_flt );
            fault_data->Num_of_BitSets = count_bits( set_flt );
            return;
         }
}


/*
int main()
{
    uchar din_hex[33], fltdin[16], okdin[16], dout[16], key[16];
    unsigned int key_schedule[60],idx;
    Hex2Char( "2b7e151628aed2a6abf7158809cf4f3c", key );
    Hex2Char( trimspaces( din_hex,
                        "39 25 84 1d 02 dc 09 fb dc 11 85 97 19 6a 0b 32"),
              okdin );
    // Hex2Char( "39252ae702dca7bbdc112bd4196aa5ba", din );

    // FAULT TEXT HERE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Hex2Char( trimspaces( din_hex,
                         "3925844d02dcd1fbdc0b8597866a0b32"),
              fltdin );
    aes_fltdecrypt( okdin, fltdin, key_schedule, 128 );
}
*/
